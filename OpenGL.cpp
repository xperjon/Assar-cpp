...

				{
...

					{
...

						{
							...
							if(pTempCellPos->m_pCell->m_fLifeTime>1)
...

						{
							...
							if(pTempCellPos->m_pCell->m_fLifeTime>1 && pTempCellPos->m_pCell->m_fDist<((CRadarStation*)pTempPos->m_pUtrustning)->m_fMaxRange)
			...
			{		
				...
				{		
					...
					glBegin(GL_LINES);
						...
						glVertex3f(pTempPos->m_pUtrustning->m_fStartPosX,pTempPos->m_pUtrustning->m_fStartPosY,0.0f);

						glVertex3f(pTempPos->m_pUtrustning->m_fWayPoints[0],pTempPos->m_pUtrustning->m_fWayPoints[1],0.0f);
...

					{
						...
						glBegin(GL_LINES);
							...
							glVertex3f(pTempPos->m_pUtrustning->m_fWayPoints[2*k-2],pTempPos->m_pUtrustning->m_fWayPoints[2*k-1],0.0f);

							glVertex3f(pTempPos->m_pUtrustning->m_fWayPoints[2*k],pTempPos->m_pUtrustning->m_fWayPoints[2*k+1],0.0f);
				...
				glVertex3f(pTempPos->m_pUtrustning->m_fPosX,pTempPos->m_pUtrustning->m_fPosY,0);
...

{
	...
	if(m_strCurrentObject=="OverView")
	{
		...
		avs=CRadarCalculate::dist(0,0,point.x*((m_fScale*6*aspect*tan((m_nZoomin/2.0f)*(M_PI/180.0f)))/tempwindow.right),point.y*((m_fScale*6*tan((m_nZoomin/2.0f)*(M_PI/180.0f))/tempwindow.bottom)));
